...
const WhyUAEParallax = () => {
  ...
  const wrapperRef = useRef(null); // <-- new: measure centered content wrapper
  ...
  useEffect(() => {
    return scrollY.onChange((latest) => {
      if (!containerRef.current || !leftSectionRef.current || !rightSectionRef.current || !wrapperRef.current) return;

      const containerTop = containerRef.current.getBoundingClientRect().top + latest;
  ...
  bg-[linear-gradient(90deg,rgba(36,43,61,1)_0%,rgba(10,14,29,1)_48%)] 
    ...
    >
      ...
      <div
        ref={wrapperRef} /* <-- attach ref to centered wrapper so measurements align with layout */
        className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-16 py-12 sm:py-16 md:py-20 lg:py-24"
      >
        {/* make left column narrower and right column wider on lg+ */}
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-8 md:gap-12 lg:gap-16 lg:min-h-screen">
          {/* LEFT SECTION - FIXED */}
          <div className="relative lg:h-screen lg:flex lg:items-start lg:pt-20">
            <motion.div
              ...
              className={`w-full lg:w-full max-w-[360px] lg:pr-8 ${isFixed ? 'lg:z-40' : 'relative'}`}
              style={
            ...
            >
              ...
              >
                <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold bg-gradient-to-r from-white via-blue-100 to-white bg-clip-text text-transparent leading-tight">
                  Why UAE
                </h2>
                {/* <div className="h-1 w-16 bg-gradient-to-r from-yellow-400 to-blue-400 rounded-full" /> */}
                <p className="text-sm sm:text-base md:text-lg text-slate-300/90 leading-relaxed font-light pr-4">
                Dubai is the region’s leading business hub, offering 100% foreign ownership, tax-free benefits, and world-class infrastructure. Its strategic location connects global markets, making it ideal for investors and entrepreneurs seeking growth through free zone, mainland, or offshore company formation opportunities.
                </p>
              </motion.div>
            ...
            {contentCards.map((card, index) => (
              ...
              >
                ...
                <div className="col-span-1 flex items-center justify-center px-4 text-center">
                  <div>
                    {card.subtitle && (
                      <p className="hidden md:block text-sm text-slate-300/80 mb-2">{card.subtitle}</p>
                    )}
                    <h3 className="text-sm md:text-lg lg:text-xl font-semibold text-white/95 leading-tight">
                      {card.title}
                    </h3>
                  </div>
                </div>
                ...
                <div className="col-span-1 px-2 md:px-6">
                  <p className="text-xl md:text-sm text-slate-300/85 leading-relaxed max-w-xl">
                    {card.description}
                  </p>
                </div>
              </motion.div>
